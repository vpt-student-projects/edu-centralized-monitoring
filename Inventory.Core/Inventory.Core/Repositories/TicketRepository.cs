...
public class TicketRepository : BaseRepository, ITicketRepository
{
    ...
    public async Task AddAsync(Ticket ticket)
    {
        ...
        cmd.Parameters.AddWithValue("@roomId",
            ticket.RoomID.HasValue ? (object)ticket.RoomID.Value : DBNull.Value);
    ...
    public async Task<Ticket?> GetByIdAsync(int ticketId)
    {
        ...
        await conn.OpenAsync();
        using var cmd = new SQLiteCommand(
            "SELECT * FROM Tickets WHERE TicketID = @id", conn);
    ...
    public async Task<IEnumerable<Ticket>> GetOpenTicketsAsync()
    {
        ...
        // ClosedStatusId = 7
        using var cmd = new SQLiteCommand(
            "SELECT * FROM Tickets WHERE StatusID != 7", conn);
    ...
    public async Task<IEnumerable<Ticket>> GetAllTicketsAsync()
    {
        ...
        await conn.OpenAsync();
        using var cmd = new SQLiteCommand(
            "SELECT * FROM Tickets ORDER BY CreatedAt DESC", conn);
        using var reader = await cmd.ExecuteReaderAsync();
        while (await reader.ReadAsync())
            tickets.Add(MapTicket((SQLiteDataReader)reader));
        return tickets;
    ...
    private Ticket MapTicket(SQLiteDataReader reader)
    {
        var ticket = new Ticket
        {
            ...
            StatusID = reader["StatusID"] == DBNull.Value
                ? 5
